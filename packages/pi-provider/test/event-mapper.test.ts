...
test("maps tool, queue, compaction, retry, and extension events", () => {
  assert.deepEqual(
    mapPiSessionEvent(
      { type: "tool_execution_end", toolCallId: "tool-1", toolName: "read", result: { content: ["ok"] } },
      1,
    ),
    [
      {
  ...
  assert.deepEqual(
    mapPiSessionEvent({ type: "extension_error", error: { message: "extension failed" }, extensionPath: "/x" }, 5),
    [
      {
