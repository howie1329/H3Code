...
const defaultSession = { mode: "create", sessionPath: undefined };
export class PiSdkProvider {
    #listeners = new Set();
    #runtimeFactory;
    #uiBridge = new PiExtensionUiBridge((event) => this.emit(event));
    #options;
    #runtime;
    #services;
    #unsubscribe;
    ...
    async start() {
        ...
        const runtime = await this.#runtimeFactory(withRuntimeDefaults({
    ...
    prompt(input) {
        ...
        return this.resolveAfterPreflight((preflightResult) => session.prompt(input.text, {
    ...
    resolveAfterPreflight(operation) {
    ...
    emitSessionReplacement(operation, cancelled) {
        if (cancelled) {
    ...
    emitRuntimeDiagnostics(runtime) {
...
function getStreamingMessage(session) {
    const state = toRecord(session.state);
    return state.streamingMessage;
}
function asQueueMode(value) {
