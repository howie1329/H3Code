...
import { deleteRegisteredSessionForRepo, listPiSessionsForRepo } from "@h3code/pi-provider";
import {
  clearAllIndexedData,
  getPreferences,
  removeIndexedRepo,
...
export class PlatformService {
  ...
  async deleteSession(input: {
    ...
    const findConnectionIdForSession = (sessionId: string) => {
      if (input.connectionId) {
        return input.connectionId;
      }

      return this.connections?.findConnectionIdForSession(sessionId);
    };

    const sessions = await deleteRegisteredSessionForRepo({
