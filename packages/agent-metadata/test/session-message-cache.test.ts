...
test("evictSessionMessageCaches keeps most recently opened sessions", () => {
  ...
  try {
    ...
    const remaining = db.prepare("SELECT session_path AS path FROM session_message_cache ORDER BY path").all() as Array<{
      path: string;
    }>;

    assert.equal(remaining.length, 3);
    assert.deepEqual(
      remaining.map((row) => row.path),
      ["/repo/.pi/sessions/2.jsonl", "/repo/.pi/sessions/3.jsonl", "/repo/.pi/sessions/4.jsonl"],
    );
  } finally {
    db.close();
  }
