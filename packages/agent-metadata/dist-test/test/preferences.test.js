...
import { getRecentRepos, migrateRecentReposSchema, migrateRepoSessionsSchema, } from "../src/preferences-schema.js";
...
test("getIndexedSessions qualifies joined session columns", () => {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run("/repo/.pi/sessions/session-a.json", "/repo", "session-a", "Session A", "2026-05-01T00:00:00.000Z", "2026-05-03T00:00:00.000Z", null, 4, "first prompt", "2026-05-03T00:01:00.000Z");
        db.prepare(`
      INSERT INTO repo_sessions (
        session_path,
        repo_path,
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run("/repo/.pi/sessions/session-b.json", "/repo", "session-b", null, "2026-05-01T00:00:00.000Z", "2026-05-02T00:00:00.000Z", null, 1, "", "2026-05-02T00:01:00.000Z");
        db.prepare(`
      INSERT INTO session_worktrees (session_path, repo_path, worktree_path, created_at)
      VALUES (?, ?, ?, ?)
    `).run("/repo/.pi/sessions/session-a.json", "/repo", "/repo/worktrees/session-a", "2026-05-03T00:02:00.000Z");
...
test("migrateRecentReposSchema backfills added order from last opened", () => {
    ...
    `);
      ...
      VALUES (?, ?, ?), (?, ?, ?)
    `).run("/repo-a", "repo-a", "2026-05-01T00:00:00.000Z", "/repo-b", "repo-b", "2026-05-02T00:00:00.000Z");
...
test("getIndexedSessions prefers recently opened sessions over older modified sessions", () => {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        insert.run("/repo/.pi/sessions/opened.json", "/repo", "opened", "Opened", "2026-05-01T00:00:00.000Z", "2026-05-01T00:00:00.000Z", "2026-05-04T00:00:00.000Z", 1, "", "2026-05-04T00:01:00.000Z");
        insert.run("/repo/.pi/sessions/modified.json", "/repo", "modified", "Modified", "2026-05-01T00:00:00.000Z", "2026-05-03T00:00:00.000Z", null, 1, "", "2026-05-03T00:01:00.000Z");
...
test("getIndexedSessions uses modified time when it is newer than last opened", () => {
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        insert.run("/repo/.pi/sessions/modified-after-open.json", "/repo", "modified-after-open", "Modified after open", "2026-05-01T00:00:00.000Z", "2026-05-05T00:00:00.000Z", "2026-05-02T00:00:00.000Z", 1, "", "2026-05-05T00:01:00.000Z");
        insert.run("/repo/.pi/sessions/opened.json", "/repo", "opened", "Opened", "2026-05-01T00:00:00.000Z", "2026-05-01T00:00:00.000Z", "2026-05-04T00:00:00.000Z", 1, "", "2026-05-04T00:01:00.000Z");
