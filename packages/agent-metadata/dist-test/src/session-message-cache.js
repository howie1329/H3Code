...
export function migrateSessionMessageCacheSchema(db) {
    db.exec(`
...
export function getSessionMessageCache(db, sessionPath) {
...
export function upsertSessionMessageCache(db, input) {
  ...
  `).run(input.sessionPath, input.repoPath, input.providerId ?? "pi", messagesJson, sessionStateJson, messageCount, sourceStats?.mtimeMs ?? input.sourceMtimeMs ?? null, sourceStats?.sizeBytes ?? input.sourceSizeBytes ?? null, contentHash, cachedAt, input.syncedAt ?? now, lastOpenedAt, syncStatus);
    evictSessionMessageCaches(db, DEFAULT_SESSION_MESSAGE_CACHE_MAX_ENTRIES);
}
export function touchSessionMessageCache(db, sessionPath) {
    const now = new Date().toISOString();
    db.prepare(`
...
export function deleteSessionMessageCache(db, sessionPath) {
    db.prepare("DELETE FROM session_message_cache WHERE session_path = ?").run(sessionPath);
}
export function clearSessionMessageCaches(db) {
    db.exec("DELETE FROM session_message_cache");
}
export function evictSessionMessageCaches(db, maxEntries = DEFAULT_SESSION_MESSAGE_CACHE_MAX_ENTRIES) {
    const countRow = db.prepare("SELECT COUNT(*) AS count FROM session_message_cache").get();
...
export function hashContent(messagesJson) {
    return createHash("sha256").update(messagesJson).digest("hex");
}
function readSourceStats(sessionPath) {
    try {
...
function rowToEntry(row) {
    ...
    let sessionState;
    try {
        messages = JSON.parse(String(row.messagesJson));
    }
    catch {
        messages = [];
    }
    if (row.sessionStateJson) {
        try {
            sessionState = JSON.parse(String(row.sessionStateJson));
        }
        catch {
    ...
    return {
        ...
        syncStatus: row.syncStatus ?? "fresh",
    };
}
...
//# sourceMappingURL=session-message-cache.js.map