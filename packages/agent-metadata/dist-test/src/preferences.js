...
export function setPiExecutablePath(executablePath) {
...
export function recordRepoUsage(repoPath, lastSessionPath) {
    const db = getDatabase();
...
function ensureRepoStub(db, repoPath) {
    const name = basename(repoPath);
...
export function recordRepoSessions(repoPath, sessions) {
...
export function recordRepoSessionRows(repoPath, sessions) {
    const db = getDatabase();
    const now = new Date().toISOString();
    db.exec("BEGIN");
    try {
        ensureRepoStub(db, repoPath);
        const lastOpenedBySessionPath = new Map(db.prepare("SELECT session_path AS path, last_opened_at AS lastOpenedAt FROM repo_sessions WHERE repo_path = ?")
            .all(repoPath)
    ...
    `);
        for (const session of sessions) {
            const lastOpenedAt = lastOpenedBySessionPath.get(session.path) ?? (session.path === repoLastSessionPath ? repoLastOpenedAt : undefined);
            insert.run(session.path, repoPath, session.id, session.name ?? null, session.created, session.modified, lastOpenedAt ?? null, session.messageCount, session.firstMessage, now);
        }
        db.prepare(`
      UPDATE recent_repos
...
export function recordSessionWorktree(repoPath, sessionPath, worktreePath) {
    const db = getDatabase();
...
export function getSessionWorktree(sessionPath) {
...
export function getRepoWorktrees(repoPath) {
...
export function getAllSessionWorktrees() {
    return getDatabase().prepare(`
    SELECT
...
export function removeSessionWorktreeMapping(sessionPath) {
    getDatabase().prepare("DELETE FROM session_worktrees WHERE session_path = ?").run(sessionPath);
}
export function removeIndexedRepo(repoPath) {
    const db = getDatabase();
    const lastSelectedRepoPath = getSetting(db, "lastSelectedRepoPath");
    const lastSelectedSessionPath = getSetting(db, "lastSelectedSessionPath");
    const repoSessionPaths = new Set(db.prepare("SELECT session_path AS path FROM repo_sessions WHERE repo_path = ?")
        .all(repoPath)
...
export function removeIndexedSession(sessionPath, options = {}) {
    const db = getDatabase();
    const lastSelectedSessionPath = getSetting(db, "lastSelectedSessionPath");
...
export function updateDesktopSettings(settings) {
    const db = getDatabase();
    ...
    const next = {
        ...
        autoConnectOnLaunch: typeof settings.autoConnectOnLaunch === "boolean" ? settings.autoConnectOnLaunch : current.autoConnectOnLaunch,
    };
...
function getRecentRepos(db) {
    return getRecentReposRows(db, recentRepoLimit);
}
function getDesktopSettings(db) {
    return {
...
function getBooleanSetting(db, key, fallback) {
...
function recordSessionOpened(db, sessionPath, openedAt) {
