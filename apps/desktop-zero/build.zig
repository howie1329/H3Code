...
const default_zero_native_path ="/Users/howardthomas/.nvm/versions/node/v25.8.1/lib/node_modules/zero-native";
const app_exe_name = "h3code-desktop-zero";
...
pub fn build(b: *std.Build) void {
    ...
    b.getInstallStep().dependOn(&agent_server_build.step);

    const run = b.addRunArtifact(exe);
    run.step.dependOn(&frontend_build.step);
    run.step.dependOn(&agent_server_build.step);
    ...
    dev.step.dependOn(&agent_server_build.step);
    ...
    package.step.dependOn(&agent_server_build.step);
