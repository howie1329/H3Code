...
export function sessionSummaryToPiSessionSummary(summary) {
...
export function snapshotToPiSessionState(snapshot) {
...
export function snapshotToPiSessionStats(snapshot) {
...
export function connectionStatusToPiStatus(state, connectionId, repoPath, message) {
    const piState = state === "connected"
        ? "connected"
        : state === "starting"
            ? "starting"
            : state === "error"
...
export function providerUiToPiRequest(connectionId, request) {
    const base = {
...
export function wrapSessionEvent(connectionId, event) {
    return { ...event, agentId: connectionId };
}
function sessionRefToId(sessionRef) {
