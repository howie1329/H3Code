import { createEmptySessionReadModel, } from "./read-model.js";
...
export function applySessionEvent(model, event) {
...
export function hydrateFromSnapshot(model, state, messages) {
    const next = cloneModel(model);
...
export function createInitialSessionReadModel() {
    return createEmptySessionReadModel();
}
function applyRunStarted(model, occurredAt) {
...
function applyRunEnded(model, messages, occurredAt) {
...
function applyRunFailed(model, errorMessage, occurredAt) {
    model.isAgentRunning = false;
    model.phase = model.isCompacting ? "compacting" : "idle";
    model.streamingMessage = null;
...
function applyTurnStarted(model, occurredAt) {
    model.latestTurn = { state: "running", startedAt: occurredAt };
    return pushActivity(model, "turn.started", "Turn started", occurredAt);
}
function applyTurnCompleted(model, message, toolResults, occurredAt) {
...
function applyMessageStreaming(model, event) {
...
function applyToolUpdated(model, event) {
...
function applyCompactionUpdated(model, event) {
...
function applyRetryUpdated(model, event) {
    if (event.phase === "start") {
        model.retry = {
            active: true,
        ...
        const attemptLabel = event.attempt !== undefined && event.maxAttempts !== undefined
...
function applyExtensionWidget(model, event) {
...
function enrichStreamingMessage(message, deltaType) {
...
function createToolExecutionMessage(tool) {
    return {
        role: "toolExecution",
...
function findRecentMessageBySignature(messages, message) {
...
function pushActivity(model, type, detail, occurredAt) {
    const activity = {
...
function pushNotification(model, message, notifyType, occurredAt) {
    const notification = {
...
export function cloneSessionReadModel(model) {
