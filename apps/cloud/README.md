...
- **Backend:** Convex (`convex/schema.ts`, `github.ts`, `workspaceRepositories.ts`, `sessions.ts`, `sandbox.ts`, `sandboxProvision.ts`, `users.ts`, `auth.config.ts`)
...
| `sessions` | Cloud agent sessions (repo, branch, status, `sandboxId`, title) |
| `messages` | Session transcript rows (user + system messages; assistant/tool via sandbox later) |
